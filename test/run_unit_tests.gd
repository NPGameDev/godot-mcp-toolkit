...
func _test_file_guard() -> void:
	_h.begin("FileGuard (Part C boundary pin)")
	# resolve_safe — project (res://) boundary.
	_h.ok(FileGuard.resolve_safe("res://x.gd").get("error") == null, "resolve_safe res:// → ok")
	_h.ok(FileGuard.resolve_safe("res://a/b/c.tscn").get("error") == null, "resolve_safe nested → ok")
	_h.ok(FileGuard.resolve_safe("res://my..thing/x.gd").get("error") == null, "resolve_safe dots-not-dotdot → ok")
	_h.ok(FileGuard.resolve_safe("res://../escape.gd").get("error") != null, "resolve_safe traversal → denied")
	_h.ok(FileGuard.resolve_safe("/etc/passwd").get("error") != null, "resolve_safe abs-unix → denied")
	_h.ok(FileGuard.resolve_safe("C:/Windows/x").get("error") != null, "resolve_safe drive-letter → denied")
	_h.ok(FileGuard.resolve_safe("\\\\server\\share\\x").get("error") != null, "resolve_safe UNC → denied")
	_h.ok(FileGuard.resolve_safe("random/x.gd").get("error") != null, "resolve_safe non-prefix → denied")
	_h.ok(FileGuard.resolve_safe("").get("error") != null, "resolve_safe empty → denied")
	# save_path multi-prefix outlier (res:// OR user://screenshots/).
	var screenshot_prefixes := ["res://", "user://screenshots/"]
	_h.ok(FileGuard.resolve_safe("user://screenshots/shot.png", screenshot_prefixes).get("error") == null,
		"resolve_safe save_path user://screenshots → ok")
	_h.ok(FileGuard.resolve_safe("res://shot.png", screenshot_prefixes).get("error") == null,
		"resolve_safe save_path res:// → ok")
	_h.ok(FileGuard.resolve_safe("user://other/x.png", screenshot_prefixes).get("error") != null,
		"resolve_safe save_path user://other → denied")
	# resolve_safe_user — user:// boundary.
	_h.ok(FileGuard.resolve_safe_user("user://saves/x.json").get("ok", false), "resolve_safe_user user:// → ok")
	_h.ok(not FileGuard.resolve_safe_user("res://x.gd").get("ok", false), "resolve_safe_user res:// → denied (wrong prefix)")
	_h.ok(not FileGuard.resolve_safe_user("user://../escape").get("ok", false), "resolve_safe_user traversal → denied")
	_h.ok(not FileGuard.resolve_safe_user("user://addons/godot_mcp_toolkit/mcp_token").get("ok", false),
	...
	for p in ["res://x.gd", "res://a/b/c.tscn", "res://addons/foo/bar.gd", "res://my..thing/x.gd",
		...
		_h.ok(FileGuard.resolve_safe(p).get("error") == null, "fixture ALLOW res://: %s" % p)
	for p in ["res://../escape.gd", "res://a/../../../escape", "../../etc/passwd", "/etc/passwd",
			"C:/Windows/x", "random/x.gd", "file:///etc/passwd"]:
		_h.ok(FileGuard.resolve_safe(p).get("error") != null, "fixture DENY res://: %s" % p)
	_h.ok(FileGuard.resolve_safe("user://x.json").get("error") != null, "fixture DENY wrong-prefix user→project")
	_h.ok(FileGuard.resolve_safe("res://x.gd", ["user://"]).get("error") != null, "fixture DENY wrong-prefix project→user")
...
func _test_file_guard_self_protect() -> void:
	_h.begin("FileGuard self-protect (concern 020)")
	...
	_h.eq(hit.get("error"), "PATH_DENIED", "plugin source file → PATH_DENIED")
	# The bare dir itself (no trailing slash) → denied via the equality arm.
	_h.eq(FileGuard.resolve_safe("res://addons/godot_mcp_toolkit").get("error"), "PATH_DENIED",
		"bare plugin dir → PATH_DENIED")
	# A nested path deeper in the subtree → denied.
	_h.eq(FileGuard.resolve_safe("res://addons/godot_mcp_toolkit/commands/script_commands.gd").get("error"),
	...
	_h.eq(FileGuard.resolve_safe("res://foo/../addons/godot_mcp_toolkit/x.gd").get("error"),
	...
	_h.ok(FileGuard.resolve_safe("res://addons/godot_mcp_toolkit_extras/x.gd").get("error") == null,
		"sibling _extras (name-prefix collision) → ALLOWED")
	# Some OTHER addon the user may legitimately edit → ALLOWED.
	_h.ok(FileGuard.resolve_safe("res://addons/other_addon/x.gd").get("error") == null,
	...
	_h.ok(FileGuard.resolve_safe("res://scenes/godot_mcp_toolkit/x.gd").get("error") == null,
...
func _test_untrusted() -> void:
	_h.begin("Untrusted (Part C boundary pin)")
	var wrapped: String = Untrusted.wrap("script", "res://x.gd", "var x = 1")
	_h.ok(wrapped.contains("<untrusted-"), "wrap → envelope present")
	_h.ok(wrapped.contains("kind=\"script\""), "wrap → kind attr present")
	_h.ok(wrapped.contains("var x = 1"), "wrap → body preserved")
	_h.eq(wrapped.count("<untrusted-"), 1, "wrap → exactly one opening envelope")
	...
	_h.ok(not nested.contains("<untrusted-deadbeef"), "wrap → inner opening tag scrubbed")
	_h.ok(not nested.contains("</untrusted>"), "wrap → inner bare closing tag scrubbed")
	_h.ok(nested.contains("[scrubbed-envelope-tag]"), "wrap → scrub placeholder present")
	_h.eq(nested.count("<untrusted-"), 1, "wrap → still exactly one real envelope after scrub")
...
func _test_repo_url_allowlist() -> void:
	_h.begin("Catalog repo-URL allowlist (concern 043)")
	# Allowed: https, case-insensitive scheme, surrounding whitespace tolerated.
	_h.ok(ExtensionCatalog.is_allowed_repo_url("https://github.com/x/y"), "https github → allowed")
	_h.ok(ExtensionCatalog.is_allowed_repo_url("https://gitlab.com/x/y"),
		"https non-github host → allowed (scheme-only, not host)")
	_h.ok(ExtensionCatalog.is_allowed_repo_url("HTTPS://github.com/x/y"), "HTTPS uppercase → allowed")
	_h.ok(ExtensionCatalog.is_allowed_repo_url("Https://github.com/x/y"), "Https mixed-case → allowed")
	_h.ok(ExtensionCatalog.is_allowed_repo_url("  https://github.com/x/y  "), "surrounding whitespace → allowed")
	# Rejected: other schemes, missing scheme, empty.
	_h.ok(not ExtensionCatalog.is_allowed_repo_url("http://github.com/x/y"), "http (no TLS) → rejected")
	_h.ok(not ExtensionCatalog.is_allowed_repo_url("file:///etc/passwd"), "file:// → rejected")
	_h.ok(not ExtensionCatalog.is_allowed_repo_url("ftp://host/x"), "ftp:// → rejected")
	_h.ok(not ExtensionCatalog.is_allowed_repo_url("javascript:alert(1)"), "javascript: → rejected")
	_h.ok(not ExtensionCatalog.is_allowed_repo_url(""), "empty string → rejected")
	_h.ok(not ExtensionCatalog.is_allowed_repo_url("github.com/x/y"), "bare host (no scheme) → rejected")
	print("")
...
func _test_compare_versions() -> void:
	_h.begin("Catalog compare_versions (numeric-lead guard)")
	# Valid numeric versions order exactly as before.
	_h.eq(ExtensionCatalog.compare_versions("1.2.3", "1.2.3"), 0, "equal → 0")
	_h.eq(ExtensionCatalog.compare_versions("1.2.3", "1.3.0"), -1, "1.2.3 < 1.3.0 → -1")
	_h.eq(ExtensionCatalog.compare_versions("2.0.0", "1.9.9"), 1, "2.0.0 > 1.9.9 → 1")
	_h.eq(ExtensionCatalog.compare_versions("1.2", "1.2.0"), 0, "missing segment treated as 0 → equal")
	_h.eq(ExtensionCatalog.compare_versions("1.10.0", "1.9.0"), 1, "1.10.0 > 1.9.0 (numeric, not lexical) → 1")
	# Pre-release/build tag: numeric lead compared, suffix ignored (no crash).
	_h.eq(ExtensionCatalog.compare_versions("1.0.0-beta", "1.0.0"), 0, "1.0.0-beta lead == 1.0.0 → 0")
	_h.eq(ExtensionCatalog.compare_versions("1.2.0", "1.3.0-rc1"), -1, "1.2.0 < 1.3.0-rc1 (lead 3) → -1")
	_h.eq(ExtensionCatalog.compare_versions("1.0.0+build5", "1.0.0"), 0, "build metadata ignored → 0")
	print("")
...
func _test_extension_path_guard() -> void:
	_h.begin("Extension path-guard (Part D)")
	...
	_h.eq(pg.get("file_path", ""), "project", "guard_project_path → path_guards.file_path=project")
	_h.eq(pg.get("slot", ""), "user", "guard_user_path → path_guards.slot=user")
	_h.ok(not MCPToolkitExtensionOptions.new("plain").to_dict().has("path_guards"),
		"no guard methods → no path_guards key")
	# Registry stores + exposes the guards.
	var reg := MCPToolkitCommandRegistry.new()
	reg.add("ext.guarded", _h.noop, MCPToolkitExtensionOptions.new("g").guard_project_path("file_path"))
	_h.eq(reg.path_guards("ext.guarded").get("file_path", ""), "project", "registry stores path_guards")
	_h.eq(reg.path_guards("unknown.method"), {}, "registry path_guards(unknown) → {}")
	...
	_h.eq(denied.get("success"), false, "dispatch rejects traversal path")
	_h.eq(denied.get("code", ""), "PATH_DENIED", "dispatch rejection code = PATH_DENIED")
	# A valid res:// path passes the guard (handler runs → _h.noop success).
	var allowed: Dictionary = await reg.call_command("ext.guarded", {"file_path": "res://ok.gd"})
	_h.eq(allowed.get("success"), true, "dispatch allows valid res:// path")
	...
	_h.eq(absent.get("success"), true, "dispatch skips absent path param")
	# user-guard rejects a res:// value.
	reg.add("ext.user", _h.noop, MCPToolkitExtensionOptions.new("u").guard_user_path("slot"))
	var bad_user: Dictionary = await reg.call_command("ext.user", {"slot": "res://nope.gd"})
	_h.eq(bad_user.get("success"), false, "user-guard rejects res:// value")
	var ok_user: Dictionary = await reg.call_command("ext.user", {"slot": "user://saves/s.json"})
	_h.eq(ok_user.get("success"), true, "user-guard allows user:// value")
	# A command with NO guards is never filtered (built-in parity).
	reg.add("ext.plain", _h.noop, MCPToolkitExtensionOptions.new("p"))
	var passthru: Dictionary = await reg.call_command("ext.plain", {"file_path": "res://../escape.gd"})
	_h.eq(passthru.get("success"), true, "no path_guards → not filtered")
...
func _test_registry() -> void:
	...
	reg.add("t.ro", _h.noop, MCPToolkitCommandOptions.new().mark_read_only())
	_h.ok(reg.is_read_only("t.ro"), "mark_read_only → is_read_only true")

	# 2. default → is_read_only false
	reg.add("t.def", _h.noop, MCPToolkitCommandOptions.new())
	_h.ok(not reg.is_read_only("t.def"), "default → is_read_only false")

	# 3. mark_scene_independent → is_active_scene_required false
	reg.add("t.si", _h.noop, MCPToolkitCommandOptions.new().mark_scene_independent())
	_h.ok(not reg.is_active_scene_required("t.si"),
			"mark_scene_independent → is_active_scene_required false")

	# 4. default → is_active_scene_required true
	_h.ok(reg.is_active_scene_required("t.def"),
			"default → is_active_scene_required true")

	# 5. mark_exclusive_execution → is_exclusive_execution true
	reg.add("t.excl", _h.noop, MCPToolkitCommandOptions.new().mark_exclusive_execution())
	_h.ok(reg.is_exclusive_execution("t.excl"),
			"mark_exclusive_execution → is_exclusive_execution true")

	# 6. mark_cancellable → is_cancellable true
	reg.add("t.canc", _h.noop, MCPToolkitCommandOptions.new().mark_cancellable())
	_h.ok(reg.is_cancellable("t.canc"), "mark_cancellable → is_cancellable true")

	# 7-8. needs_serialization: read-only bypasses, default serialises
	_h.ok(not reg.needs_serialization("t.ro"),
			"needs_serialization for read-only → false")
	_h.ok(reg.needs_serialization("t.def"),
	...
	reg.add("t.exclmut",
			_h.noop, MCPToolkitCommandOptions.new().mark_exclusive_execution())
	_h.ok(reg.needs_serialization("t.exclmut"),
			"exclusive + non-read-only mutator → needs_serialization true")
	_h.ok(not reg.is_read_only("t.exclmut"),
	...
	reg.add("t.romut", _h.noop, MCPToolkitCommandOptions.new().mark_read_only())
	_h.ok(not reg.needs_serialization("t.romut"),
			"read-only-only command bypasses the lock — a mutator must not be "
			+ "read-only")

	# 9. remove → has_command false
	reg.add("t.rm", _h.noop, MCPToolkitCommandOptions.new())
	reg.remove("t.rm")
	_h.ok(not reg.has_command("t.rm"), "remove → has_command false")

	# 10. clear → get_all_methods empty
	var reg2 := MCPToolkitCommandRegistry.new()
	reg2.add("t.a", _h.noop, MCPToolkitCommandOptions.new())
	reg2.add("t.b", _h.noop, MCPToolkitCommandOptions.new())
	reg2.clear()
	_h.eq(reg2.get_all_methods().size(), 0, "clear → get_all_methods empty")

	# 11. mark_extension → get_extension_methods includes it
	reg.add("t.ext", _h.noop, MCPToolkitCommandOptions.new())
	reg.mark_extension("t.ext")
	_h.ok(reg.get_extension_methods().has("t.ext"),
			"mark_extension → in get_extension_methods")

	# 12. get_command_metadata contains description
	reg.add("t.desc", _h.noop,
			MCPToolkitCommandOptions.new().with_description("Hello"))
	_h.eq(reg.get_command_metadata("t.desc").get("description", ""), "Hello",
			"get_command_metadata → correct description")

	# 13. duplicate registration — overwrites cleanly, latest wins
	reg.add("t.dup", _h.noop, MCPToolkitCommandOptions.new())
	reg.add("t.dup", _h.noop, MCPToolkitCommandOptions.new().mark_read_only())
	_h.ok(reg.has_command("t.dup"), "duplicate → still registered")
	_h.ok(reg.is_read_only("t.dup"), "duplicate → latest options win")

	# 14. non-existent command — safe fallback
	_h.ok(not reg.has_command("t.nope"), "non-existent → has_command false")
	_h.eq(reg.get_command_metadata("t.nope"), {},
			"non-existent → metadata empty dict")
	_h.ok(reg.needs_serialization("t.nope"),
...
func _test_project_key() -> void:
	_h.begin("ProjectKey identity")

	# 1. Normalization: backslash → slash, trailing slash(es) stripped.
	_h.eq(ProjectKey.canonical("C:\\a\\b"), "c:/a/b" if _case_folds() else "C:/a/b",
			"canonical: backslash → slash")
	_h.eq(ProjectKey.canonical("/a/b/"), "/a/b", "canonical: trailing slash stripped")
	_h.eq(ProjectKey.canonical("/a/b///"), "/a/b", "canonical: repeated trailing slashes stripped")
	_h.eq(ProjectKey.canonical(""), "", "canonical: empty stays empty")

	# 2. Case-fold is filesystem-conditional (Windows/macOS lowercase, else verbatim).
	if _case_folds():
		_h.eq(ProjectKey.canonical("/A/B"), "/a/b", "canonical: lowercased on case-insensitive FS")
	else:
		_h.eq(ProjectKey.canonical("/A/B"), "/A/B", "canonical: case preserved on case-sensitive FS")
	...
	_h.eq(h.length(), 12, "hash_of: 12 chars")
	_h.eq(ProjectKey.hash_of("/some/canonical/key"), h, "hash_of: deterministic")
	_h.ok(ProjectKey.hash_of("/some/canonical/key") != ProjectKey.hash_of("/other/key"),
			"hash_of: distinct inputs → distinct hashes")
	_h.ok(ProjectKey.hash_of("/A/B") != ProjectKey.hash_of("/a/b"),
			"hash_of: hashes verbatim (does not re-canonicalize)")

	# 4. current_hash() == hash_of(current()) by construction.
	_h.eq(ProjectKey.current_hash(), ProjectKey.hash_of(ProjectKey.current()),
	...
	_h.eq(ProjectPaths.project_hash(), ProjectKey.current_hash(),
...
func _test_registry_paths() -> void:
	_h.begin("RegistryPaths layout")

	# 1. projects.json is the aggregate file under the registry dir.
	_h.ok(RegistryPaths.registry_path().ends_with("projects.json"),
			"registry_path ends with projects.json")
	_h.eq(RegistryPaths.registry_path(),
	...
	_h.ok(RegistryPaths.entry_dir().ends_with("entries"),
			"entry_dir ends with entries")
	_h.eq(RegistryPaths.entry_file_path(),
			RegistryPaths.entry_dir().path_join(h + ".json"),
			"entry_file_path == entry_dir/<hash>.json")
	_h.eq(RegistryPaths.runtime_entry_file_path(),
	...
	_h.ok(RegistryPaths.entry_file_path() != RegistryPaths.runtime_entry_file_path(),
			"editor entry path != runtime entry path")

	# 4. The lock file is the registry path + ".lock".
	_h.eq(RegistryPaths.lock_path(), RegistryPaths.registry_path() + ".lock",
			"lock_path == registry_path + .lock")

	# 5. The façade still routes through here (callers bind to RegistryClient).
	_h.eq(RegistryClient.registry_dir(), RegistryPaths.registry_dir(),
...
func _test_registry_entry() -> void:
	_h.begin("RegistryEntryFile build_entry")
	...
	_h.eq(e.get("lsp_host", ""), "127.0.0.1", "entry carries lsp_host")
	_h.eq(e.get("lsp_port", -1), 6005, "entry carries lsp_port")

	# 2. WS port stays distinct from the LSP port; core keys present.
	_h.eq(e.get("port", -1), 6550, "entry carries ws port (distinct from lsp_port)")
	_h.eq(e.get("token_path", ""), "tok", "entry carries token_path")
	_h.ok(e.has("_key") and e.has("pid") and e.has("started_at"),
			"entry carries core keys (_key/pid/started_at)")
	_h.ok(e.get("runtime_port") == null, "no runtime → runtime_port null")
	...
	_h.eq(e2.get("lsp_port", -1), 6010, "custom lsp_port flows through")
	_h.eq(e2.get("runtime_port", -1), 6570, "runtime_port preserved when set")
...
func _test_registry_entry_file_io() -> void:
	_h.begin("RegistryEntryFile write/read/delete")
	...
	_h.eq(RegistryEntryFile.read(tmp), {}, "read(nonexistent) → {}")
	...
	_h.eq(got.size(), entry.size(), "write then read → same field count")
	_h.eq(got.get("_key", ""), "res://proj", "round-trip: _key preserved")
	_h.eq(int(got.get("port", -1)), 6550, "round-trip: port (JSON floats ints → int())")
	_h.eq(got.get("token_path", ""), "tok", "round-trip: token_path preserved")
	_h.ok(got.get("runtime_port", 0) == null, "round-trip: runtime_port null preserved")

	# 3. delete → the file is gone, so read returns empty again.
	RegistryEntryFile.delete(tmp)
	_h.eq(RegistryEntryFile.read(tmp), {}, "delete then read → {}")
...
func _test_registry_merge() -> void:
	_h.begin("RegistryClient entry merge")
	...
	_h.eq(row.get("runtime_port", -1), 6570, "overlay: runtime_port from runtime file")
	_h.eq(row.get("runtime_pid", -1), 4242, "overlay: runtime_pid from runtime file")
	# Editor-owned fields are NOT clobbered by the runtime file's placeholders.
	_h.eq(row.get("port", -99), 6550, "overlay: editor port preserved (not -1)")
	_h.eq(row.get("token_path", "x"), "tok", "overlay: editor token_path preserved (not empty)")
	_h.eq(row.get("lsp_port", -1), 6005, "overlay: editor lsp_port preserved (not null)")
	_h.ok(not row.has("_key"), "overlay: _key erased from row")
	...
	_h.eq(orow.get("runtime_port", -1), 6570, "runtime-only: runtime_port present")
	_h.eq(orow.get("port", -99), -1, "runtime-only: port -1")
	_h.eq(orow.get("token_path", "x"), "", "runtime-only: token_path empty")
	_h.ok(orow.has("godot_version"), "runtime-only: godot_version present (schema-complete)")
	_h.eq(orow.get("lsp_port", -99), null, "runtime-only: lsp_port null")
	_h.ok(not orow.has("_key"), "runtime-only: _key erased")
	...
	_h.eq(erow.get("runtime_port", -99), null, "editor-only: runtime_port null")
	_h.eq(erow.get("runtime_pid", -99), null, "editor-only: runtime_pid null")
	_h.eq(erow.get("port", -99), 6550, "editor-only: editor port preserved")
	_h.ok(not erow.has("_key"), "editor-only: _key erased")
	...
	_h.eq(cleared.get("res://proj", {}).get("runtime_port", -99), null,
...
func _test_extension_collision_guard() -> void:
	_h.begin("Extension-load collision guard")
	var reg := MCPToolkitCommandRegistry.new()
	...
	reg.add("scene.create_node", _h.noop, MCPToolkitCommandOptions.new().mark_read_only())
	reg.add("acme.do_thing", _h.noop, MCPToolkitCommandOptions.new())
	...
	reg.add("scene.create_node", _h.noop, MCPToolkitCommandOptions.new())  # tries to hijack
	var r1 := reg.end_extension_load()
	_h.eq(r1.size(), 1, "built-in collision → one refusal recorded")
	_h.eq(str(r1[0].get("method", "")), "scene.create_node", "refusal names the colliding command")
	_h.ok(reg.is_read_only("scene.create_node"),
			"built-in incumbent untouched (options preserved, not overwritten)")
	_h.ok(not reg.get_extension_methods().has("scene.create_node"),
	...
	reg.begin_extension_load()
	reg.add("shared.tool", _h.noop, MCPToolkitCommandOptions.new().mark_idempotent())  # ext A — lands
	var r_a := reg.end_extension_load()
	reg.mark_extension("shared.tool")
	_h.eq(r_a.size(), 0, "ext A first add of a new name → not refused")
	_h.ok(reg.has_command("shared.tool"), "ext A's command is registered")

	reg.begin_extension_load()
	reg.add("shared.tool", _h.noop, MCPToolkitCommandOptions.new())  # ext B — refused
	var r_b := reg.end_extension_load()
	_h.eq(r_b.size(), 1, "ext B duplicate of the same new name → refused (first-writer-wins)")
	_h.ok(reg.get_command_metadata("shared.tool")["annotations"]["idempotentHint"],
	...
	reg.begin_extension_load()
	reg.add("acme.do_thing", _h.noop, MCPToolkitCommandOptions.new())  # collides → refused
	reg.add("acme.brand_new", _h.noop, MCPToolkitCommandOptions.new())  # new → lands
	var r3 := reg.end_extension_load()
	_h.eq(r3.size(), 1, "mixed load → exactly the colliding name refused")
	_h.eq(str(r3[0].get("method", "")), "acme.do_thing", "the colliding name is the refused one")
	_h.ok(reg.has_command("acme.brand_new"), "non-colliding add in the same load still succeeds")
	...
	reg.begin_extension_load()
	reg.add("acme.brand_new", _h.noop, MCPToolkitCommandOptions.new())  # re-add own just-removed name
	var r4 := reg.end_extension_load()
	_h.eq(r4.size(), 0, "re-adding a just-removed own name → not refused")
	_h.ok(reg.has_command("acme.brand_new"), "extension re-registers its own command after removal")
	...
	reg.add("acme.brand_new", _h.noop, MCPToolkitCommandOptions.new().mark_read_only())
	_h.ok(reg.is_read_only("acme.brand_new"),
...
func _test_extension_support() -> void:
	_h.begin("Extension support (candidate + addon-enabled)")

	# is_extension_candidate — GDScript matched by base class.
	_h.ok(ExtensionSupport.is_extension_candidate({"base": "MCPToolkitExtension"}),
			"GDScript base == MCPToolkitExtension → candidate")
	# C# matched by MCPToolkit prefix on a .cs path (can't extend the GDScript base).
	_h.ok(ExtensionSupport.is_extension_candidate({"class": "MCPToolkitFoo", "path": "res://foo.cs"}),
			"C# MCPToolkit-prefixed .cs → candidate")
	# Negatives: prefix without .cs, .cs without prefix, unrelated base, empty.
	_h.ok(not ExtensionSupport.is_extension_candidate({"class": "MCPToolkitFoo", "path": "res://foo.gd"}),
			"MCPToolkit-prefixed but .gd (no GDScript base) → not a candidate")
	_h.ok(not ExtensionSupport.is_extension_candidate({"class": "PlainCs", "path": "res://plain.cs"}),
			"non-prefixed .cs → not a candidate")
	_h.ok(not ExtensionSupport.is_extension_candidate({"base": "RefCounted", "class": "Internal"}),
			"unrelated base class → not a candidate")
	_h.ok(not ExtensionSupport.is_extension_candidate({}),
			"empty entry → not a candidate")

	# is_addon_enabled — a script outside res://addons/ has no addon toggle → enabled.
	_h.ok(ExtensionSupport.is_addon_enabled("res://my_ext.gd"),
			"non-addon path → enabled")
	_h.ok(ExtensionSupport.is_addon_enabled("res://scenes/foo/bar.gd"),
	...
	_h.ok(ExtensionSupport.is_addon_enabled("res://addons/_nonexistent_addon_xyz/ext.gd"),
...
func _assert_buttons(buttons: Array, expected: Array, label: String) -> void:
	...
	_h.eq(buttons.size(), expected.size(), "%s → button count" % label)
	for i in expected.size():
		if i >= buttons.size():
			break
		var got: Dictionary = buttons[i]
		var want: Array = expected[i]
		_h.eq(str(got.get("label", "")), str(want[0]), "%s → button %d label" % [label, i])
		_h.eq(str(got.get("action", "")), str(want[1]), "%s → button %d action" % [label, i])


func _test_onboarding_wizard_specs() -> void:
	_h.begin("Onboarding wizard step specs")
	...
	_h.ok(not str(s0.get("text", "")).is_empty(), "step 0 → text non-empty")
	_h.eq(str(s0.get("ok_label", "")), "Next", "step 0 → ok_label")
	_assert_buttons(s0.get("buttons", []),
			[["Open Security Doc", "open_security"]], "step 0")
	_h.ok(not s0.has("on_enter"), "step 0 → no on_enter")

	# Step 1 variant A — .mcp.json EXISTS: keep-existing OK + an overwrite button.
	var s1e: Dictionary = wiz._spec_mcp_json(true)
	_h.ok(not str(s1e.get("text", "")).is_empty(), "step 1 (exists) → text non-empty")
	_h.ok(str(s1e.get("text", "")).contains("already exists"),
			"step 1 (exists) → names the existing-file case")
	_h.eq(str(s1e.get("ok_label", "")), "Continue (keep existing .mcp.json)",
	...
	_h.ok(str(s1n.get("text", "")).contains("No .mcp.json was found"),
			"step 1 (absent) → names the missing-file case")
	_h.eq(str(s1n.get("ok_label", "")), "Create .mcp.json",
	...
	_h.ok(not str(s2.get("text", "")).is_empty(), "step 2 → text non-empty")
	_h.eq(str(s2.get("ok_label", "")), "Close", "step 2 → ok_label")
	_assert_buttons(s2.get("buttons", []),
			[["Back", "back"], ["Open Info", "open_info"]], "step 2")
	_h.ok(s2.get("on_enter") is Callable, "step 2 → on_enter is a Callable")

	# Dispatcher routes by _step and records _mcp_exists when step 1 renders.
	wiz._step = 0
	_h.eq(str(wiz._spec_for_step().get("ok_label", "")), "Next", "dispatch step 0 → welcome spec")
	wiz._step = 1
	...
	if wiz._mcp_exists:
		_h.eq(str(d1.get("ok_label", "")), "Continue (keep existing .mcp.json)",
				"dispatch step 1 → spec matches _mcp_exists=true")
	else:
		_h.eq(str(d1.get("ok_label", "")), "Create .mcp.json",
				"dispatch step 1 → spec matches _mcp_exists=false")
	wiz._step = 2
	_h.eq(str(wiz._spec_for_step().get("ok_label", "")), "Close", "dispatch step 2 → dock spec")
...
func _test_build_command_entry() -> void:
	_h.begin("build_command_entry (extensions.list/refresh/changed wire shape)")
	...
	_h.eq(full.get("method", ""), "ext.full", "full → method seed present")
	_h.eq(full.get("description", ""), "Full entry", "full → description present")
	_h.ok(full.has("input_schema") and not full.get("input_schema", {}).is_empty(),
			"full → input_schema present (non-empty)")
	_h.ok(full.has("annotations"), "full → annotations present (registry always sets them)")
	_h.ok(full.has("group") and full.get("group", {}).get("name", "") == "grp",
			"full → group present with name")
	_h.ok(full.get("group", {}).get("keywords", []).has("kw1"),
			"full → group keywords carried through (the refresh hint source)")
	_h.eq(full.get("timeout_ms", -1), 5000, "full → non-default timeout_ms present")
	...
	_h.eq(minimal.get("method", ""), "ext.minimal", "minimal → method seed present")
	_h.ok(not minimal.has("description"), "minimal → description omitted (empty)")
	_h.ok(not minimal.has("input_schema"), "minimal → input_schema omitted (empty)")
	_h.ok(not minimal.has("group"), "minimal → group omitted (empty)")
	_h.ok(not minimal.has("timeout_ms"), "minimal → timeout_ms omitted (default)")
	_h.ok(minimal.has("annotations"), "minimal → annotations present (always built)")
	...
	_h.eq(unknown.size(), 1, "unknown method → entry holds only the method seed")
	_h.eq(unknown.get("method", ""), "ext.unknown", "unknown method → method seed present")
...
func _test_compute_class_diff() -> void:
	_h.begin("Watcher set-diff kernel (concern 047 C5)")

	# A class only in `current` → added; carries its path.
	var d1 := ExtensionWatcher.compute_class_diff(
		{"NewExt": "res://new.gd"}, {}, {})
	_h.ok(d1["added"].has("NewExt"), "class only in current → added")
	_h.eq(d1["added"].get("NewExt", ""), "res://new.gd", "added carries the script path")
	_h.ok(d1["removed"].is_empty(), "nothing known → removed empty")
	_h.ok(d1["retry"].is_empty(), "nothing failed → retry empty")

	# A class only in `known` → removed (by name); not added.
	var d2 := ExtensionWatcher.compute_class_diff(
		{}, {"GoneExt": "res://gone.gd"}, {})
	_h.ok("GoneExt" in d2["removed"], "class only in known → removed")
	_h.ok(d2["added"].is_empty(), "nothing current → added empty")
	...
	var d3 := ExtensionWatcher.compute_class_diff(
		...
		{"FixedExt": "res://fixed.gd"},
		{"FixedExt": true})
	_h.ok(d3["retry"].has("FixedExt"), "failed ∩ current (known) → retry")
	_h.eq(d3["retry"].get("FixedExt", ""), "res://fixed.gd", "retry carries the script path")
	_h.ok(d3["added"].is_empty(), "already known → not added")
	_h.ok(d3["removed"].is_empty(), "still present → not removed")
	...
	_h.ok(d4["added"].has("FlakyExt"), "failed + new → added")
	_h.ok(d4["retry"].is_empty(), "failed + new → NOT retry (excluded by added)")
	...
	var d5 := ExtensionWatcher.compute_class_diff(
		...
		{"StableExt": "res://stable.gd"},
		{})
	_h.ok(d5["added"].is_empty(), "unchanged → not added")
	_h.ok(d5["removed"].is_empty(), "unchanged → not removed")
	_h.ok(d5["retry"].is_empty(), "unchanged → not retried")

	# Empty inputs → empty delta on every axis.
	var d6 := ExtensionWatcher.compute_class_diff({}, {}, {})
	_h.ok(d6["added"].is_empty() and d6["removed"].is_empty() and d6["retry"].is_empty(),
...
func _test_options_builder() -> void:
	_h.begin("Options builder")

	# 1-5. Boolean marks
	_h.ok(MCPToolkitCommandOptions.new().mark_read_only().to_dict()["is_read_only"],
			"mark_read_only → to_dict is_read_only true")
	_h.ok(MCPToolkitCommandOptions.new().mark_destructive().to_dict()["is_destructive"],
			"mark_destructive → to_dict is_destructive true")
	_h.ok(MCPToolkitCommandOptions.new().mark_idempotent().to_dict()["is_idempotent"],
			"mark_idempotent → to_dict is_idempotent true")
	_h.ok(MCPToolkitCommandOptions.new().mark_exclusive_execution().to_dict() \
			.get("exclusive_execution", false),
			"mark_exclusive_execution → to_dict exclusive_execution true")
	_h.ok(MCPToolkitCommandOptions.new().mark_cancellable().to_dict()["is_cancellable"],
			"mark_cancellable → to_dict is_cancellable true")

	# 6. with_timeout_ms
	_h.eq(MCPToolkitCommandOptions.new().with_timeout_ms(5000).to_dict()["timeout_ms"],
			5000, "with_timeout_ms(5000) → 5000")

	# 7. chained builder returns same reference
	var opts := MCPToolkitCommandOptions.new()
	_h.ok(opts.mark_read_only().mark_idempotent() == opts,
	...
	_h.eq(g.get("name", ""), "grp", "with_group → name")
	_h.eq(g.get("description", ""), "Desc", "with_group → description")
	_h.ok(g.get("keywords", []).has("kw"), "with_group → keywords")

	# 9-10. Version gating
	_h.eq(MCPToolkitCommandOptions.new().with_min_godot_version("4.5") \
			.to_dict().get("min_godot_version", ""), "4.5",
			"with_min_godot_version → '4.5'")
	_h.eq(MCPToolkitCommandOptions.new().with_max_godot_version("4.4") \
	...
	_h.ok(vd.has("min_godot_version") and vd.has("max_godot_version"),
			"chained version bounds → both present")

	# 12. invalid version string — stored despite push_warning
	_h.eq(MCPToolkitCommandOptions.new().with_min_godot_version("bad") \
...
func _test_extension_options() -> void:
	...
	_h.ok(not fresh["is_read_only"], "default → not read-only")
	_h.ok(not fresh["is_destructive"], "default → not destructive")
...
func _test_annotation_mapping() -> void:
	_h.begin("Annotation mapping")
	var reg := MCPToolkitCommandRegistry.new()

	# 1. mark_read_only → readOnlyHint true
	reg.add("a.ro", _h.noop, MCPToolkitCommandOptions.new().mark_read_only())
	_h.ok(reg.get_command_metadata("a.ro")["annotations"]["readOnlyHint"],
			"mark_read_only → readOnlyHint true")

	# 2. mark_destructive → destructiveHint true
	reg.add("a.ds", _h.noop, MCPToolkitCommandOptions.new().mark_destructive())
	_h.ok(reg.get_command_metadata("a.ds")["annotations"]["destructiveHint"],
			"mark_destructive → destructiveHint true")

	# 3. mark_idempotent → idempotentHint true
	reg.add("a.id", _h.noop, MCPToolkitCommandOptions.new().mark_idempotent())
	_h.ok(reg.get_command_metadata("a.id")["annotations"]["idempotentHint"],
			"mark_idempotent → idempotentHint true")

	# 4. no marks → all hints false
	reg.add("a.plain", _h.noop, MCPToolkitCommandOptions.new())
	var ann: Dictionary = reg.get_command_metadata("a.plain").get("annotations", {})
	_h.ok(not ann.get("readOnlyHint", false), "no marks → readOnlyHint false")
	_h.ok(not ann.get("destructiveHint", false), "no marks → destructiveHint false")
	_h.ok(not ann.get("idempotentHint", false), "no marks → idempotentHint false")
...
func _test_timeout_clamping() -> void:
	_h.begin("Timeout clamping")
	var reg := MCPToolkitCommandRegistry.new()

	# 1. no timeout → default 30000 (metadata omits key)
	reg.add("to.def", _h.noop, MCPToolkitCommandOptions.new())
	_h.ok(not reg.get_command_metadata("to.def").has("timeout_ms"),
			"no timeout → default 30000 (omitted from metadata)")

	# 2. below min → clamped to 1000
	reg.add("to.lo", _h.noop, MCPToolkitCommandOptions.new().with_timeout_ms(500))
	_h.eq(reg.get_command_metadata("to.lo").get("timeout_ms", -1), 1000,
			"timeout 500 → clamped to 1000")

	# 3. above max → clamped to 300000
	reg.add("to.hi", _h.noop, MCPToolkitCommandOptions.new().with_timeout_ms(500000))
	_h.eq(reg.get_command_metadata("to.hi").get("timeout_ms", -1), 300000,
			"timeout 500000 → clamped to 300000")

	# 4. in range → unchanged
	reg.add("to.ok", _h.noop, MCPToolkitCommandOptions.new().with_timeout_ms(5000))
	_h.eq(reg.get_command_metadata("to.ok").get("timeout_ms", -1), 5000,
			"timeout 5000 → unchanged")

	# 5. zero → default (same as no timeout)
	reg.add("to.z", _h.noop, MCPToolkitCommandOptions.new().with_timeout_ms(0))
	_h.ok(not reg.get_command_metadata("to.z").has("timeout_ms"),
