...
static func _test_set_property_compound(h) -> void:
	h.begin("set_property_compound")
	...
	h.ok(r1.get("ok", false), "theme_override slash path → ok")
	h.eq(ctrl.get("theme_override_font_sizes/font_size"), 24,
	...
	h.ok(r2.get("ok", false), "shader_parameter colon path → ok")
	var readback = sprite.get("material").get_shader_parameter("brightness")
	h.eq(readback, 0.3, "shader_parameter readback = 0.3")
	sprite.free()
	...
	h.ok(not r3.get("ok", false), "null sub-resource → error")
	h.eq(r3.get("code", ""), "NOT_FOUND", "error code = NOT_FOUND")
	node.free()
...
static func _test_compound_set_helper(h) -> void:
	h.begin("compound_set helper")
	...
	h.eq(ctrl.get("theme_override_font_sizes/font_size"), 32,
	...
	h.eq(mat.get_shader_parameter("brightness"), 0.4,
	...
	h.eq(mat.get_shader_parameter("brightness"), 0.75,
	...
	h.eq(pass2.get_shader_parameter("glow"), 0.5,
	...
	h.eq(node.visible, false, "simple: visible set to false")
	node.free()
	...
	h.ok(true, "null sub-resource: no crash")
	empty.free()
...
static func _test_undo_info(h) -> void:
	h.begin("_undo info")
	...
	var r1 := Helpers.set_property_compound(
		ctrl, "theme_override_font_sizes/font_size", 24)
	h.ok(r1.get("ok", false), "slash-only: set ok")
	var u1: Dictionary = r1.get("_undo", {})
	h.eq(u1.get("type"), "property", "slash-only: _undo type = property")
	h.eq(u1.get("path"), "theme_override_font_sizes/font_size",
	...
	var r2 := Helpers.set_property_compound(
		sprite, "material:shader_parameter/brightness", 0.3)
	h.ok(r2.get("ok", false), "colon: set ok")
	var u2: Dictionary = r2.get("_undo", {})
	h.ok(u2.get("type") == "property" or u2.get("type") == "sub_resource",
		"colon: _undo type is property or sub_resource")
	h.eq(u2.get("old"), null, "colon: _undo old = null (no prior override)")
	sprite.free()
