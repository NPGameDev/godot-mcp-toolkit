...
static func _test_registry(h) -> void:
	h.begin("Registry")
	var reg := MCPToolkitCommandRegistry.new()

	# 1. mark_read_only → is_read_only true
	reg.add("t.ro", h.noop, MCPToolkitCommandOptions.new().mark_read_only())
	h.ok(reg.is_read_only("t.ro"), "mark_read_only → is_read_only true")

	# 2. default → is_read_only false
	reg.add("t.def", h.noop, MCPToolkitCommandOptions.new())
	h.ok(not reg.is_read_only("t.def"), "default → is_read_only false")

	# 3. mark_scene_independent → is_active_scene_required false
	reg.add("t.si", h.noop, MCPToolkitCommandOptions.new().mark_scene_independent())
	h.ok(not reg.is_active_scene_required("t.si"),
			"mark_scene_independent → is_active_scene_required false")

	# 4. default → is_active_scene_required true
	h.ok(reg.is_active_scene_required("t.def"),
			"default → is_active_scene_required true")

	# 5. mark_exclusive_execution → is_exclusive_execution true
	reg.add("t.excl", h.noop, MCPToolkitCommandOptions.new().mark_exclusive_execution())
	h.ok(reg.is_exclusive_execution("t.excl"),
			"mark_exclusive_execution → is_exclusive_execution true")

	# 6. mark_cancellable → is_cancellable true
	reg.add("t.canc", h.noop, MCPToolkitCommandOptions.new().mark_cancellable())
	h.ok(reg.is_cancellable("t.canc"), "mark_cancellable → is_cancellable true")

	# 7-8. needs_serialization: read-only bypasses, default serialises
	h.ok(not reg.needs_serialization("t.ro"),
			"needs_serialization for read-only → false")
	h.ok(reg.needs_serialization("t.def"),
	...
	reg.add("t.exclmut",
			h.noop, MCPToolkitCommandOptions.new().mark_exclusive_execution())
	h.ok(reg.needs_serialization("t.exclmut"),
			"exclusive + non-read-only mutator → needs_serialization true")
	h.ok(not reg.is_read_only("t.exclmut"),
	...
	reg.add("t.romut", h.noop, MCPToolkitCommandOptions.new().mark_read_only())
	h.ok(not reg.needs_serialization("t.romut"),
			"read-only-only command bypasses the lock — a mutator must not be "
			+ "read-only")

	# 9. remove → has_command false
	reg.add("t.rm", h.noop, MCPToolkitCommandOptions.new())
	reg.remove("t.rm")
	h.ok(not reg.has_command("t.rm"), "remove → has_command false")

	# 10. clear → get_all_methods empty
	var reg2 := MCPToolkitCommandRegistry.new()
	reg2.add("t.a", h.noop, MCPToolkitCommandOptions.new())
	reg2.add("t.b", h.noop, MCPToolkitCommandOptions.new())
	reg2.clear()
	h.eq(reg2.get_all_methods().size(), 0, "clear → get_all_methods empty")

	# 11. mark_extension → get_extension_methods includes it
	reg.add("t.ext", h.noop, MCPToolkitCommandOptions.new())
	reg.mark_extension("t.ext")
	h.ok(reg.get_extension_methods().has("t.ext"),
			"mark_extension → in get_extension_methods")

	# 12. get_command_metadata contains description
	reg.add("t.desc", h.noop,
			MCPToolkitCommandOptions.new().with_description("Hello"))
	h.eq(reg.get_command_metadata("t.desc").get("description", ""), "Hello",
			"get_command_metadata → correct description")

	# 13. duplicate registration — overwrites cleanly, latest wins
	reg.add("t.dup", h.noop, MCPToolkitCommandOptions.new())
	reg.add("t.dup", h.noop, MCPToolkitCommandOptions.new().mark_read_only())
	h.ok(reg.has_command("t.dup"), "duplicate → still registered")
	h.ok(reg.is_read_only("t.dup"), "duplicate → latest options win")

	# 14. non-existent command — safe fallback
	h.ok(not reg.has_command("t.nope"), "non-existent → has_command false")
	h.eq(reg.get_command_metadata("t.nope"), {},
			"non-existent → metadata empty dict")
	h.ok(reg.needs_serialization("t.nope"),
