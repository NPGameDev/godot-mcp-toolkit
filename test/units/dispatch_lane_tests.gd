...
static func run(testing) -> void:
	_test_watchdog_timeout(testing)
	_test_scene_lease(testing)
...
static func _test_scene_lease(testing) -> void:
	testing.begin("Scene lease acquire/release bookkeeping")
	...
	testing.ok(lease.try_acquire(peer_a, ""), "free lease → A acquires")
	testing.ok(lease.lease_holder() == peer_a, "lease holder is A")

	# 2. same peer → renews
	testing.ok(lease.try_acquire(peer_a, ""), "same peer → renews (true)")
	testing.ok(lease.lease_holder() == peer_a, "A still holds after renew")

	# 3. different peer → contended (false); A keeps it
	testing.ok(not lease.try_acquire(peer_b, ""), "other peer → contended (false)")
	testing.ok(lease.lease_holder() == peer_a, "A still holds under contention")

	# 4. release → no holder
	lease.release()
	testing.ok(lease.lease_holder() == null, "release → no holder")

	# 5. after release → B acquires
	testing.ok(lease.try_acquire(peer_b, ""), "after release → B acquires")
	testing.ok(lease.lease_holder() == peer_b, "lease holder is B")
...
static func _test_mutation_watchdog(testing) -> void:
...
static func _test_summarize_batch(testing) -> void:
	testing.begin("summarize_batch batch partial-failure rollup")
	...
	testing.ok(not r_all_ok.has("failed"), "all-success → no failed key (additive: unchanged)")
	testing.ok(not r_all_ok.has("hint"), "all-success → no hint key")
	testing.eq(r_all_ok.get("count"), 2, "all-success → pre-existing keys preserved")
	...
	testing.eq(r_one_fail.get("failed"), 1, "1-of-3 success:false → failed=1")
	testing.ok(str(r_one_fail.get("hint", "")).contains("1 of 3"), "hint reports 1 of 3")
	testing.ok(str(r_one_fail.get("hint", "")).contains("results[]"), "hint steers to results[]")
	...
	testing.eq(r_groups.get("failed"), 1, "site-2 (no success key) error entry → counted")
	testing.eq(r_groups.get("count"), 2, "site-2 → pre-existing count preserved")
	testing.eq(r_groups.get("action"), "add", "site-2 → pre-existing action preserved")
	...
	testing.ok(not r_all_added.has("failed"), "site-2 all-status (no error) → no failed key")

	# Empty results → UNCHANGED.
	var empty := {"results": [], "count": 0}
	var r_empty := Helpers.summarize_batch(empty, "results")
	testing.ok(not r_empty.has("failed"), "empty results → no failed key")
	testing.ok(not r_empty.has("hint"), "empty results → no hint key")
