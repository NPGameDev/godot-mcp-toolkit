...
static func run(h) -> void:
	_test_spatial_map(h)
	_test_texture_generate(h)
	_test_particle_prop_apply(h)
	_test_particle_merge_overrides(h)
	_test_sound_generate(h)
	_test_tileset_edit_key_enforcement(h)
	_test_tileset_io_polygon(h)
...
static func _test_spatial_map(h) -> void:
	...
	h.ok(typeof(b3) == TYPE_AABB, "Node3D → AABB")
	h.ok(b3.size == Vector3.ZERO, "Node3D → point AABB (zero size; world pos via interactive)")
	n3.free()

	var n2 := Node2D.new()
	n2.position = Vector2(5, 6)
	var b2 = SpatialCommands._world_bounds(n2)
	h.ok(typeof(b2) == TYPE_RECT2, "Node2D → Rect2")
	h.ok(b2.position == Vector2(5, 6), "Node2D Rect2 at global_position")
	n2.free()
	...
	h.ok(typeof(bc) == TYPE_RECT2, "Control → Rect2")
	h.ok(bc.size == Vector2(20, 30), "Control Rect2 size from get_global_rect")
	ctrl.free()

	var plain := Node.new()
	h.ok(SpatialCommands._world_bounds(plain) == null, "plain Node → null (non-spatial)")
	plain.free()

	# _xform_rect2 / _xform_aabb world-space transform.
	h.ok(SpatialCommands._xform_rect2(Transform2D.IDENTITY, Rect2(0, 0, 10, 10)) == Rect2(0, 0, 10, 10),
		"_xform_rect2 identity → same")
	var rt = SpatialCommands._xform_rect2(Transform2D(0.0, Vector2(5, 5)), Rect2(0, 0, 10, 10))
	h.ok(rt.position == Vector2(5, 5) and rt.size == Vector2(10, 10), "_xform_rect2 translate")
	h.ok(SpatialCommands._xform_aabb(Transform3D.IDENTITY, AABB(Vector3.ZERO, Vector3(2, 2, 2)))
	...
	h.ok(entries[0]["overlaps"].has("b"), "overlap a-b detected")
	h.ok(not entries[0]["overlaps"].has("c"), "no overlap a-c (disjoint)")
	h.ok(entries[0]["contains"].has("d"), "containment a contains d")
	h.ok(entries[3]["contained_by"].has("a"), "containment d contained_by a")
	h.ok(entries[0].has("nearest"), "nearest neighbour computed (full)")
	...
	h.ok(mixed[0]["overlaps"].is_empty(), "2D node never overlaps 3D node")

	# Region parsing + filtering.
	h.ok(typeof(SpatialCommands._parse_region([0, 0, 10, 10])) == TYPE_RECT2, "_parse_region 4 nums → Rect2")
	h.ok(typeof(SpatialCommands._parse_region([0, 0, 0, 1, 1, 1])) == TYPE_AABB, "_parse_region 6 nums → AABB")
	h.ok(SpatialCommands._parse_region([1, 2, 3]).has("error"), "_parse_region bad size → error")
	h.ok(SpatialCommands._parse_region(null) == null, "_parse_region null → null")
	h.ok(SpatialCommands._passes_filters(Rect2(0, 0, 5, 5), Rect2(0, 0, 10, 10), null),
		"region: 2D node inside → pass")
	h.ok(not SpatialCommands._passes_filters(Rect2(0, 0, 5, 5), AABB(Vector3.ZERO, Vector3.ONE), null),
		"region: 3D region excludes 2D node")

	# Serialization.
	h.eq(SpatialCommands._vec_to_array(Vector2(1, 2)), [1.0, 2.0], "_vec_to_array Vector2")
...
static func _test_texture_generate(h) -> void:
	...
	h.ok(TextureCommands._parse_color(null, Color(0.5, 0.5, 0.5, 1)) == Color(0.5, 0.5, 0.5, 1),
		"_parse_color null → default")
	var c_hex = TextureCommands._parse_color("#ff0000", Color.BLACK)
	h.ok(c_hex.r > 0.99 and c_hex.g < 0.01 and c_hex.b < 0.01, "_parse_color #ff0000 → red")
	h.ok(TextureCommands._parse_color([0, 255, 0], Color.BLACK).g > 0.99, "_parse_color [0,255,0] → green (0-255)")
	h.ok(TextureCommands._parse_color([0, 0, 1], Color.BLACK).b > 0.99, "_parse_color [0,0,1] → blue (0-1)")
	h.ok(TextureCommands._parse_color([0, 0, 0, 0], Color.WHITE).a == 0.0,
		"_parse_color [0,0,0,0] → transparent (alpha-absent)")

	# _in_shape inside/outside.
	h.ok(TextureCommands._in_shape("solid", 8, 8, 16, 16, "up", 0), "solid: center inside")
	h.ok(TextureCommands._in_shape("circle", 8, 8, 16, 16, "up", 0), "circle: center inside")
	h.ok(not TextureCommands._in_shape("circle", 0, 0, 16, 16, "up", 0), "circle: corner outside")
	h.ok(TextureCommands._in_shape("diamond", 8, 8, 16, 16, "up", 0), "diamond: center inside")
	h.ok(not TextureCommands._in_shape("diamond", 0, 0, 16, 16, "up", 0), "diamond: corner outside")
	h.ok(TextureCommands._in_shape("triangle", 8, 14, 16, 16, "up", 0), "triangle(up): bottom-center inside")
	h.ok(not TextureCommands._in_shape("triangle", 1, 1, 16, 16, "up", 0), "triangle(up): top-corner outside")
	...
	h.ok(solid.get_pixel(8, 8) == red, "solid fill: center red")
	h.ok(solid.get_pixel(0, 0) == red, "solid fill: corner red (covers all)")
	...
	h.ok(circ.get_pixel(8, 8) == red, "circle fill: center red")
	h.ok(circ.get_pixel(0, 0).a == 0.0, "circle: corner transparent (background)")
	...
	h.ok(hollow.get_pixel(0, 0) == blue, "hollow solid: border blue (outline band)")
	h.ok(hollow.get_pixel(8, 8).a == 0.0, "hollow solid: interior transparent (no fill)")
	...
	h.ok(checker.get_pixel(0, 0) == red, "checkerboard: cell (0,0) fill")
...
static func _test_particle_prop_apply(h) -> void:
	...
	h.eq(n_all, 15, "all 15 props present → count 15")

	# Node group values + casts.
	h.eq(node_all.amount, 24, "amount float 24.0 → int 24")
	h.ok(typeof(node_all.amount) == TYPE_INT, "amount cast to int")
	h.ok(is_equal_approx(node_all.lifetime, 1.5), "lifetime → 1.5")
	h.ok(is_equal_approx(node_all.explosiveness, 0.5), "explosiveness → 0.5")
	h.ok(is_equal_approx(node_all.speed_scale, 2.0), "speed_scale → 2.0")
	h.eq(node_all.one_shot, true, "one_shot int 1 → bool true")
	h.eq(node_all.local_coords, true, "local_coords → true")

	# Material group values + casts.
	h.eq(mat_all.direction, dir, "direction → Vector3 (raw assign)")
	h.ok(is_equal_approx(mat_all.spread, 30.0), "spread int 30 → float 30.0")
	h.eq(mat_all.gravity, grav, "gravity → Vector3 (raw assign)")
	h.eq(mat_all.color, col, "color → Color (raw assign)")
	h.eq(mat_all.particle_flag_align_y, true, "particle_flag_align_y → true")
	h.ok(is_equal_approx(mat_all.emission_sphere_radius, 12.0), "emission_sphere_radius → 12.0")
	h.eq(mat_all.emission_box_extents, box, "emission_box_extents → Vector3 (raw assign)")
	h.eq(mat_all.turbulence_enabled, true, "turbulence_enabled → true")
	h.ok(is_equal_approx(mat_all.turbulence_noise_strength, 0.75), "turbulence_noise_strength → 0.75")
	node_all.free()
	...
	h.eq(ParticleCommands._apply_props(node_none, mat_none, {}), 0, "empty eff → count 0")
	h.eq(node_none.amount, amount_default, "empty eff → amount untouched")
	h.eq(mat_none.spread, spread_default, "empty eff → spread untouched")
	node_none.free()
	...
	h.eq(n_sub, 3, "subset of 3 → count 3")
	h.ok(is_equal_approx(node_sub.lifetime, 3.0), "subset: lifetime landed")
	h.ok(is_equal_approx(mat_sub.spread, 45.0), "subset: spread landed")
	h.eq(mat_sub.turbulence_enabled, true, "subset: turbulence_enabled landed")
	# A prop absent from the subset eff must NOT have been counted/written.
	h.eq(node_sub.amount, node_none.amount, "subset: absent amount stays default")
	node_sub.free()
...
static func _test_particle_merge_overrides(h) -> void:
	...
	h.eq(ov_none.size(), 0, "no params → overrides_applied empty")
	h.eq(eff_pre, fire, "no params → eff identical to preset")
	...
	h.eq(ov_sub, ["amount", "spread", "initial_velocity"], "shadowing overrides in contract order")
	h.eq(eff_sub["amount"], 99, "amount float 99.0 → int 99")
	h.ok(typeof(eff_sub["amount"]) == TYPE_INT, "amount cast to int")
	h.ok(is_equal_approx(eff_sub["spread"], 5.0), "spread int 5 → float 5.0")
	h.ok(is_equal_approx(eff_sub["initial_velocity_min"], 1.0), "range min landed")
	h.ok(is_equal_approx(eff_sub["initial_velocity_max"], 2.0), "range max landed")
	...
	h.eq(ov_vec.size(), 0, "no preset → vec/colour applied but nothing shadowed")
	h.eq(eff_vec["direction"], Vector3(0, -1, 0), "direction dict → Vector3")
	h.eq(eff_vec["color"], Color(0.2, 0.4, 0.6, 0.8), "color dict → Color")
	h.eq(eff_vec["gravity"], Vector3(0, 49, 0), "gravity dict → Vector3")
	...
	h.eq(ov_np.size(), 0, "empty eff → nothing shadowed → overrides empty")
	h.eq(eff_np["amount"], 7, "no-preset: amount still written to eff")
	h.ok(is_equal_approx(eff_np["scale_min"], 2.0), "scalar range → scale_min")
	h.ok(is_equal_approx(eff_np["scale_max"], 2.0), "scalar range → scale_max == min")
	h.ok(not eff_np.has("lifetime"), "absent param never written")
	...
	h.eq(ov_ord, ["color", "scale_range", "emission_shape"], "order: simple → range → emission")
...
static func _test_sound_generate(h) -> void:
	...
	h.ok(abs(SoundCommands._oscillator("sine", 0.0)) < 0.001, "sine(0) approx 0")
	h.ok(SoundCommands._oscillator("sine", PI / 2.0) > 0.99, "sine(pi/2) approx 1")
	h.ok(SoundCommands._oscillator("square", 0.5) == 1.0, "square(+) = 1")
	h.ok(SoundCommands._oscillator("square", PI + 0.5) == -1.0, "square(-) = -1")
	h.ok(SoundCommands._oscillator("sawtooth", 0.0) < -0.99, "sawtooth(0) approx -1")
	h.ok(SoundCommands._oscillator("triangle", PI / 2.0) > 0.99, "triangle(pi/2) approx 1")
	...
	h.eq(pcm.size(), expected_samples * 2, "_build_pcm byte length = samples*2 (16-bit mono)")
	...
	for i in range(mid, mini(mid + 120, expected_samples)):
		if pcm.decode_s16(i * 2) != 0:
			found_nonzero = true
			break
	h.ok(found_nonzero, "_build_pcm sine non-silent in sustain")
	...
	for i in range(silent.size() / 2):
		if silent.decode_s16(i * 2) != 0:
			all_zero = false
			break
	h.ok(all_zero, "_build_pcm volume 0 → silence")
...
static func _test_tileset_edit_key_enforcement(h) -> void:
	...
	h.eq(TilesetTileData._foreign_key_error("custom_data",
	...
	h.eq(TilesetTileData._foreign_key_error("physics", {"atlas_x": 0, "atlas_y": 0}),
	...
	h.ok(not r1.is_empty(), "terrain_set on physics → rejected")
	h.ok(r1.contains("tileset.edit_terrain"), "physics rejection names tileset.edit_terrain")

	var r2 := TilesetTileData._foreign_key_error("terrain",
		{"atlas_x": 0, "atlas_y": 0, "physics_polygon": "full"})
	h.ok(r2.contains("tileset.edit_physics"), "physics_polygon on terrain → names edit_physics")

	var r3 := TilesetTileData._foreign_key_error("navigation",
		{"atlas_x": 0, "atlas_y": 0, "probability": 0.5})
	h.ok(r3.contains("tileset.edit_visuals"), "probability on navigation → names edit_visuals")

	var r4 := TilesetTileData._foreign_key_error("custom_data",
		{"atlas_x": 0, "atlas_y": 0, "navigation_polygon": "full"})
	h.ok(r4.contains("tileset.edit_navigation"), "navigation_polygon on custom_data → names edit_navigation")

	var r5 := TilesetTileData._foreign_key_error("visuals",
		{"atlas_x": 0, "atlas_y": 0, "custom_data": {"x": 1}})
	h.ok(r5.contains("tileset.edit_custom_data"), "custom_data on visuals → names edit_custom_data")
	...
	h.ok(not r6.is_empty(), "unknown key on physics → rejected")
	h.ok(r6.contains("unknown key"), "unknown-key rejection uses unknown-key wording")

	# Unknown verb has an empty allow-list → first non-coord key is foreign.
	h.ok(not TilesetTileData._foreign_key_error("bogus_verb",
...
static func _test_tileset_io_polygon(h) -> void:
	...
	h.eq(p_rect, PackedVector2Array([
...
static func _test_create_collision_resolver(h) -> void:
	...
	h.eq(c_create.get("valid"), true, "absent + return → valid")
	h.eq(c_create.get("existed"), false, "absent + return → existed false")
	h.eq(c_create.get("action"), "create", "absent + return → action create")
	h.eq(Helpers.resolve_create_collision(absent, "fail").get("action"), "create",
		"absent + fail → action create (value irrelevant when absent)")
	h.eq(Helpers.resolve_create_collision(absent, "replace").get("action"), "create",
		"absent + replace → action create")

	# Invalid if_exists → {valid:false} (no existence read needed).
	h.eq(Helpers.resolve_create_collision(absent, "clobber").get("valid"), false,
		"invalid value 'clobber' → valid false")
	h.eq(Helpers.resolve_create_collision(absent, "").get("valid"), false,
		"empty value → valid false")
	h.eq(Helpers.resolve_create_collision(absent, "Return").get("valid"), false,
	...
	if f == null:
		h.ok(false, "could not open temp file for existence cases — SKIPPED exists path")
	else:
		f.store_string("x")
		f.close()

		var c_return := Helpers.resolve_create_collision(present, "return")
		h.eq(c_return.get("valid"), true, "exists + return → valid")
		h.eq(c_return.get("existed"), true, "exists + return → existed true")
		h.eq(c_return.get("action"), "return", "exists + return → action return")
		h.eq(Helpers.resolve_create_collision(present, "fail").get("action"), "fail",
			"exists + fail → action fail")
		h.eq(Helpers.resolve_create_collision(present, "replace").get("action"), "replace",
		...
		h.eq(Helpers.resolve_create_collision(present, "nope").get("valid"), false,
