...
> **Param name (both tools):** `tilemap_set_cells` and `tilemap_read_cells` both take
> `node_path` — they name the target node consistently. If either accepts a
> `tilemap_path` key, the canonical rename has regressed.
