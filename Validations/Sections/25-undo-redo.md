...
- **Constraint:** drive UR1.4→UR1.6 (set→undo→redo) tightly within the same session/dispatch chain. A redo issued across *separate* dispatch round-trips or separate test sessions can hit editor-history contention (a different `history_id` context) and appear to no-op — an env/sequencing artifact, not a production bug.
