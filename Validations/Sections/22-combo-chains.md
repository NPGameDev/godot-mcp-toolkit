...
`tileset_create` (res://sv2_validation/c6_ts.tres, texture_path=res://icon.svg, tile_size={32,32}, physics=true) → `tileset_setup_layers` (terrain_sets=[{mode:"match_corners_and_sides", terrains:["grass"]}]) → `scene_create_node` (TileMapLayer [4.3+] or TileMap [4.2], "C6Tile") → `node_set_property` (tile_set={"type":"Resource","path":"res://sv2_validation/c6_ts.tres"}) → `tilemap_set_cells` (node_path=`C6Tile`, regions=[{x:0,y:0,width:3,height:3,source_id:0,atlas_x:0,atlas_y:0}]) → cleanup (`scene_delete_node` C6Tile, `resource_delete` c6_ts.tres)
