...
- `power_user` profile recommended for full coverage (all tools available)
...
3. **Exception:** Core tools (scene, node, editor, project) are always available on power_user profile — no activation needed.
...
|---|------|-------|-------|---------------|--------------|
| 0 | [00-environment.md](Sections/00-environment.md) | Environment Detection | 5 | project_get_settings, asset_list, discover_tools | None |
...
| 3 | [03-node-properties.md](Sections/03-node-properties.md) | Node Properties & Methods | 22 | node_set/get_property, node_set_script, node_get_property_list, node_call_method | S2 |
...
| 12 | [12-classdb.md](Sections/12-classdb.md) | ClassDB Introspection | 4 | classdb_search, classdb_get_info | None |
...
| 14 | [14-tileset-tilemap.md](Sections/14-tileset-tilemap.md) | TileSet & TileMap | 14 | tileset_create, tileset_edit, tilemap_set_cells | S2 |
...
The sweep includes **REGRESSION WATCH** annotations for 23 DX fixes + 6 known bug fixes from the 41k validation cluster. These appear inline in the relevant section files. If a test marked with REGRESSION WATCH fails, it indicates a previously-fixed behavior has regressed — flag prominently.

| Category | Count | Sections |
|----------|-------|----------|
| FIX-1 through FIX-9 | 9 | S1, S3, S7, S9, S14 |
| FIX-A through FIX-K | 11 | S2, S3, S5, S8, S14, S22 |
| Pitfalls 3, 4, 6 | 3 (canary) | S3, S22 |
| Other fixes (commit-ref) | 6 | S2, S3, S4, S5, S7, S20, S21 |
