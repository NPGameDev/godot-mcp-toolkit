...
## Sweep facts (41n-nonies, Godot 4.2 floor — 2026-07-10, PARTIAL S0–S7, handed off)
- **Environment preflight BEFORE S0 (two traps that cost a full run of orchestrator turns):** detect the running Godot version via **ClassDB** (`classdb_search("TileMap")`: TileMapLayer present ⇒ ≥4.3, absent ⇒ 4.2), NOT the boot banner (stale-log prone), the features PSA (project floor), or scene_close catalog-presence → tool-sweep-delegation.md §Environment preflight.
- **Headless-editor trap:** 2×2/81-byte stub screenshots + live `push_warning` not captured in the buffer + `RendererDummy`/`DummyMesh` in the log ⇒ the MCP is bound to a HEADLESS Godot; screenshot/console/runtime results are invalid and every console-isolation check is VACUOUS — connect a GUI editor & re-verify before trusting them → tool-sweep-delegation.md §Environment preflight.
- **Prep briefs must NOT bake a `discover_tools` activate/reset step** (orchestrator owns groups; an agent `reset=true` kills the always-on groups). Give each RUN agent a 'skip any discover_tools step' override. Same-message group-activate + spawn-prep DOES work.
