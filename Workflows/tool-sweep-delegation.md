...
## Environment preflight (learned 41n-nonies, 2026-07-10 — two traps that burned a full run)
Run BOTH checks in the orchestrator's INTERACTIVE session BEFORE spawning S0, and bake the confirmed facts into `_common.md` so every brief inherits them:

1. **Version via ClassDB, not the banner.** `classdb_search(pattern:"TileMap", instantiable_only:false)` — `TileMapLayer` in the results ⇒ engine ≥4.3; only legacy `TileMap` (no TileMapLayer) ⇒ **4.2**. Do NOT trust: the `application/config/features` PSA (it is the project's declared FLOOR — it read "4.2" here while a stale banner said 4.5), the boot banner via `editor_get_console(source:file)` (can be a STALE log — look for the `"fallback to stale log — no post-boot log found"` warning), or `scene_close` being in the tool catalog (registered on ALL versions; returns UNSUPPORTED only at call-time on <4.5). Getting the version wrong mislabels the whole run and every version-gated Expect line. The 4.2 profile: scene_close→UNSUPPORTED, TileMapLayer unavailable (use legacy `TileMap`), toasts skipped, extensions need a restart, `script_check` omits line/col (leaner diagnostics — the prior "line:0 stub" FAIL does NOT reproduce), the preload→`load()` hint may not fire (S6.5).

2. **GUI editor, not headless.** Before trusting ANY screenshot/console/runtime result: `editor_screenshot()` must return a real large PNG (NOT a 2×2 / 81-byte stub), AND a fresh `execute_code(push_warning("PROBE"))` must appear in `editor_get_console(source:buffer)`. If screenshots are stubs AND live warnings aren't captured AND the log shows `RendererDummy`/`DummyMesh` (the headless renderer) or a past `run_unit_tests.gd` run, the MCP is bound to a **HEADLESS** Godot — commonly a `--headless` test/CI process that grabbed port 6550 before the GUI editor. Symptoms: blank screenshots (S7/S20/S28); the console buffer never captures live output so **every section's console-isolation check is VACUOUS**; S20/S21/S27 runtime/debugger break. This is NOT a plugin bug and is NOT fixed by unlocking the PC or restarting into a GUI editor if the headless process still holds 6550. Fix: close the headless `godot` process (Task Manager / the terminal running the test suite), leave only the GUI editor with the plugin enabled, `/mcp` reconnect, re-verify the two probes. Pin `GODOT_MCP_EDITOR_PORT` if multiple editors contend for the port.

Also confirmed 41n-nonies: **prep briefs must NOT include any `discover_tools` activate/reset step** — the orchestrator owns all group management; a section-runner calling `discover_tools(reset=true)` deactivates the always-on `cleanup`/`resource_io`/`editor_advanced`/`asset_ops` groups and breaks every later section. Tell prep to OMIT any group-reset the section spec lists, and give each RUN agent an explicit 'skip any discover_tools step' override. Same-message `discover_tools(request:[...])` + spawn-prep DOES propagate (the prep's ToolSearch sees the freshly-activated group).
