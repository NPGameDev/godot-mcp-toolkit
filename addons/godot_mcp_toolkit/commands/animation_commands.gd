@tool
extends RefCounted
## animation.* command handlers — add/remove/get keyframes on AnimationPlayer tracks.
...
static func register(registry: MCPCommandRegistry, server: Node) -> void:
	registry.add("animation.add_key", func(parameters: Dictionary) -> Dictionary:
		return _cmd_animation_add_key(server, parameters), "full")
	registry.add("animation.remove_key", func(parameters: Dictionary) -> Dictionary:
		return _cmd_animation_remove_key(server, parameters), "full")
...
static func _cmd_animation_add_key(
	...
	var time_raw = parameters.get("time", 0.0)
	...
	if track_path.is_empty() or not track_path.contains(":"):
		return MCPError.make("INVALID_PARAMS",
			"track_path must include a property (e.g. 'Sprite2D:position'); method/transform/bezier tracks not supported in 15d")
	if not track_type_param.is_empty() and track_type_param != "value":
		return MCPError.make("INVALID_PARAMS",
			"track_type='%s' not supported in 15d (only 'value' / default); transform2d/transform3d/bezier/method/audio deferred" % track_type_param)
...
static func _cmd_animation_remove_key(
	server: Node, parameters: Dictionary,
) -> Dictionary:
	var player_path := str(parameters.get("player_path", ""))
	var animation_name := str(parameters.get("animation_name", ""))
