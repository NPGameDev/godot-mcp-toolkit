...
static func compose(plugin: EditorPlugin, on_user_path_changed: Callable) -> Handle:
	...
	var export_plugin := preload("res://addons/godot_mcp_toolkit/export_strip.gd").new()
	...
	var user_path_monitor = _Hub.UserPathMonitor.new()
	...
	# -- Bottom-panel dock --
	var dock: Control = preload("res://addons/godot_mcp_toolkit/ui/dock.tscn").instantiate()
	dock.bind(server, _Hub.Audit.get_log_path())
