@tool
extends RefCounted
## Pure guidance for the dock's macOS "listening, but no client connected" hint.
##
...
## surfaces the one-shot nudge and its message text; the dock owns the session
## state (ever-connected, already-shown) and the toast. Deliberately NOT coupled to
## NodejsCheck: that check reads green via its own login-shell fallback while a GUI
## client still can't find Node, so the trigger is no-peer-after-grace, never
## node-not-found.

# Grace a client is given to connect before the no-peer nudge fires (~20s) —
...
## True iff the dock should surface the macOS no-peer launch hint now. The Q4
...
## NOT [param already_shown] (a dismissible one-shot). Any single false → silent.
...
## The one-shot hint text: the launchd minimal-PATH cause plus the three recovery
